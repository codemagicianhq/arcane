...
2. **EF-36 (auto-merge fires on CI green with no notion of an open review round) is filed HIGH, deferred pending an ADR.** Confirmed 2026-08-24/25: `allow_auto_merge` is currently `false` on the repo (was `true` at the 2026-08-23 incident) — lowers immediate risk, does not resolve the design question. Still the top next action — see `TODO.md`'s *PR Workflow and Prompt Integrity* section.
