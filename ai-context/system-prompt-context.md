...
8. **Codex Support is the active program (CS-00–CS-08), activated 2026-09-09; CS-00 through CS-03 are merged and `1.0.0` is published.** [docs/plans/codex-support/PLAN.md](../docs/plans/codex-support/PLAN.md), PRD at `features/codex-support/PRD.md`, architecture at `features/codex-support/architecture.md`. Merged 2026-09-09: CS-00 ([PR #220](https://github.com/codemagicianhq/arcane/pull/220)), CS-01 ([PR #221](https://github.com/codemagicianhq/arcane/pull/221), `0.39.0` — Codex ships), CS-02 ([PR #226](https://github.com/codemagicianhq/arcane/pull/226); ARC-045 accepted in [PR #230](https://github.com/codemagicianhq/arcane/pull/230)), the CHANGELOG catch-up ([PR #231](https://github.com/codemagicianhq/arcane/pull/231)), and **CS-03 — the canonical move to `.arcane/spells/`, every client a generated shim — as [PR #232](https://github.com/codemagicianhq/arcane/pull/232), operator-merged, released as `v1.0.0` and on npm** (`publish.yml` confirmed, tarball verified). One operator item stays open in `docs/plans/codex-support/OPERATOR-QUEUE.md`: Q-004, the one-minute Copilot Chat check of the shim — the one client no agent session can observe; its fallback (an inlined-body Copilot renderer) is a small follow-up epic, not a redesign. **Next eligible epic: CS-04 (user tier install)** via `docs/plans/codex-support/KICKOFF.md`, whose first design call is the user-tier CLI surface (`--user` flag vs a `spell user` noun — a Naming Test, ARC-045's open question 1); CS-08 (the restore-model spike, research only) may run any time now that the canonical folder exists. Two standing rules for this program: every show-report regeneration is a trailer-free commit (TODO.md's Show Report item), and never open a second PR that regenerates this program's report while another is open — whichever merges second must rebase and regenerate first.
...
- **Active task:** Codex Support — CS-03 is merged and published (`1.0.0`); the next eligible epic is
  CS-04, user tier install — `docs/plans/codex-support/PLAN.md ("CS-04 — User tier install")`. Not
  started.
- **Last completed step:** Recorded CS-03's confirmed merge and publish — PLAN.md's CS-03 box ticked
  with merge commit `a68c974` and the publish run, `OPERATOR-QUEUE.md` Q-003 marked done with the same
  evidence, priority 8 refreshed — and pushed that record for its own docs PR.
- **Next concrete action:** Run `docs/plans/codex-support/KICKOFF.md`'s loop for CS-04
  (`docs/plans/codex-support/PLAN.md ("CS-04 — User tier install")`): its first design call is the
  user-tier CLI surface (`--user` flag vs a `spell user` noun — a Naming Test, ARC-045's open question
  1), then `spell-full-cycle` end to end. CS-08 (`docs/plans/codex-support/PLAN.md ("CS-08 —
  Distribution-model spike")`) may run any time now that the canonical folder exists.
- **Active files:** None uncommitted after the merge-record commit.
- **Branch:** `main` (the merge-record branch is deleted once its PR merges).
- **Blockers:** None. Q-004 (the operator's one-minute Copilot Chat check —
  `docs/plans/codex-support/OPERATOR-QUEUE.md ("Q-004 — Confirm the Copilot shim in VS Code (one minute)")`)
  is informational for CS-04, not a gate: if Copilot paraphrases the shim, the fix is an inlined-body
  Copilot renderer as a small follow-up epic.
- **Pending Verification:** Copilot Chat shim behavior — unverifiable from any agent session — Q-004's
  steps in VS Code. Nothing else in flight: `1.0.0` is on npm (`npm view arcane-cli version`), PR #232
  is merged (`gh pr view 232 --json state,mergeCommit`).
- **Notes:** Three standing facts for this program. (1) Every show-report regeneration is a
  trailer-free commit, and never two open PRs regenerating `docs/plans/codex-support/show-report.*`
  (`TODO.md ("the golden-parity gate has no notion of an in-progress program")`). (2) `codex exec`
  ...
  (3) The CS-04 fan-out must not use `copier.ts` (its target-path guard forbids leaving the target
  directory); the OpenClaw precedent in `agent-generator.ts` is the pattern — see the plan's CS-04
  section.
