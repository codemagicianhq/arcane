...
- **Active task:** None. The Show Report program closed at SR-08 (`docs/plans/show-report/PLAN.md`,
  `status: complete`, 2026-09-05) with a GO verdict and two disclosed deviations, both registered:
  `TODO.md ("MEDIUM: Show Report's share lens (was SR-05c)")` and SR-07 below.
- **Last completed step:** Merged [PR #214](https://github.com/codemagicianhq/arcane/pull/214), the
  program close, and confirmed CI green on `main` at `994bc67`. npm serves `0.38.2`, matching
  `package.json`.
- **Next concrete action:** Nothing in Show Report is startable here. Either (a) enable SR-07 by
  following `docs/sr-07-enable.md` in the **arcane-ui** repo and opening a PR for the pushed branch
  `feat/sr-07-export-template-pr`, or (b) pick up the highest-value unrelated item —
  `TODO.md ("MEDIUM: Claude Code worktree branches bypass Arcane's branch-naming standard")`, whose
  operator decision is already recorded (rename-on-sight, ARC-023), so it only needs implementing.
- **Active files:** None uncommitted. Last edits: `docs/plans/show-report/PLAN.md`, `project.md`,
  `ai-context/system-prompt-context.md`, `TODO.md`,
  `journal/2026-09-05-show-report-sr05-sr08-close.md`.
- **Branch:** `docs/session-close-2026-09-05` (this close's own branch; `main` after it merges).
- **Blockers:** None in this repository. SR-07 is blocked on an operator-created GitHub token and an
  Azure DevOps secret — platform mutations outside every delegation.
- **Pending Verification:** arcane-ui `feat/sr-07-export-template-pr` — dispatched — pushed with one
  commit not on `main` and no PR opened there; verify with `git -C ../arcane-ui log --oneline
  origin/main..origin/feat/sr-07-export-template-pr`, then open a PR in that repo if you want it.
- **Notes:** The transferable lesson from this program is that **a byte-comparison gate cannot catch
  a wrong template** — mustache renders an unknown tag as an empty string, silently, so a template
  whose tags name nothing still regenerates byte-identically. `npm run check:report-template` exists
  because four separate arcane-ui builds passed every other gate while broken; keep it blocking. One
  Definition-of-Done criterion was deliberately left unverified and is stated as such in the plan's
  close-out audit: "prints to PDF cleanly" — the print block exists and measures above 4.5:1, but no
  PDF was produced. That is a minute in a browser and it is the operator's to make.
