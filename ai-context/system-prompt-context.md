...
- **Notes:** (1) The operator's machine has **no** user tier installed: the session's live check was
  ...
  operator's decision (Q-005). (2) Every show-report regeneration is a trailer-free commit, and never
  two open PRs regenerating `docs/plans/codex-support/show-report.*`
