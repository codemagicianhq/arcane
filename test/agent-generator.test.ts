...
const ORCHESTRATOR_DEF: AgentDefinition = {
  ...
  persona: {
    ...
    catchphrases: ["Places, everyone — the curtain is up.", "A good show is run from the wings, not the spotlight.", "Every act goes on when I say it goes on."],
  },
  model: { primary: "openai-codex", fallback: [] },
  tools: { allowed: ["shell", "git"], denied: ["sudo"] },
  autonomy: { default_power_level: "Magus", exec_allowed: true },
...
describe("syncAgents — dry-run", () => {
  it("returns synced items and does not write files", async () => {
    ...
    await expect(
      readFile(join(githubAgentsDir, "kellar.agent.md"), "utf8"),
    ).rejects.toThrow();
  });
});
...
describe("syncAgents — Copilot output", () => {
  it("creates .github/agents/{slug}.agent.md for each agent with copilot.agent_file=true", async () => {
    ...
    const kellarAgent = await readFile(
      join(tmpDir, ".github", "agents", "kellar.agent.md"),
      "utf8",
    );
    const lafayetteAgent = await readFile(
      join(tmpDir, ".github", "agents", "lafayette.agent.md"),
      "utf8",
    );

    expect(kellarAgent).toContain("Kellar");
    expect(kellarAgent).toContain("Product Operations Manager");
    expect(kellarAgent).toContain("Places, everyone — the curtain is up.");
    expect(kellarAgent).toContain("Commanding and principled");
    expect(kellarAgent).toContain("Direct, measured, and strategic");
    expect(lafayetteAgent).toContain("Lafayette");
    expect(lafayetteAgent).toContain("Full-Stack Developer");
  });
...
describe("syncAgents — OpenClaw output", () => {
  it("writes IDENTITY.md, SOUL.md, TOOLS.md into the correct workspace dirs", async () => {
    ...
    expect(identity).toContain("Kellar");
    expect(identity).toContain("Product Operations Manager");
    expect(identity).toContain("Places, everyone — the curtain is up.");

    const soul = await readFile(
      join(openclawRoot, "workspace", "SOUL.md"),
      "utf8",
    );
    expect(soul).toContain("Magus");
  ...
  it("writes Lafayette's files to workspace-lafayette", async () => {
    const roster = makeRoster(openclawRoot);
    await syncAgents(tmpDir, assetsDir, roster, {
      copilot: false,
      claude: false,
    ...
    const lafayetteIdentity = await readFile(
      join(openclawRoot, "workspace-lafayette", "IDENTITY.md"),
      "utf8",
    );
    expect(lafayetteIdentity).toContain("Lafayette");
  });
  ...
  it("generates openclaw-roster.json patch file", async () => {
    ...
    expect(patch.agents[0]?.identity.name).toBe("Kellar");
    expect(patch.agents[1]?.id).toBe("lafayette");
    expect(patch.agents[1]?.identity.name).toBe("Lafayette");
  });
