...
describe("applyCodemagicianNames", () => {
  it("maps orchestrator to Primus", () => {
    const result = applyCodemagicianNames(["orchestrator"]);
    expect(result).toEqual([{ definition: "orchestrator", name: "Primus" }]);
  });
  ...
  it("maps all 12 standard roles to their persona names", () => {
    ...
    const result = applyCodemagicianNames(roleIds);

    const expectedNames = [
      "Primus", "Gandalf", "Thor", "Snape", "Scotty", "Wasp",
      "Flash", "Vision", "Wanda", "Trinity", "Gideon", "Heimdall",
    ];
  ...
  it("falls back to the raw id for an unknown role", () => {
    const result = applyCodemagicianNames(["unknown-role-xyz"]);
    expect(result[0]?.name).toBe("unknown-role-xyz");
  });
  ...
  it("returns an empty array for empty input", () => {
    expect(applyCodemagicianNames([])).toEqual([]);
  });
});
...
describe("applyNamingStrategy", () => {
  ...
  it("delegates to applyCodemagicianNames for 'codemagician' strategy", async () => {
    const result = await applyNamingStrategy("codemagician", ["orchestrator"]);
    expect(result).toEqual([{ definition: "orchestrator", name: "Primus" }]);
  });
  ...
  it("returns empty array for any strategy when no roles are provided", async () => {
    ...
    expect(await applyNamingStrategy("codemagician", [])).toEqual([]);
