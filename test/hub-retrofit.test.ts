...
describe("offerRegistryScaffold", () => {
  ...
  it("scaffolds registry.json from existing venture folders, excluding _template", async () => {
    await fs.mkdir(join(tmpDir, "ventures", "kiubo-mexico"), { recursive: true });
    await fs.mkdir(join(tmpDir, "ventures", "stillface"), { recursive: true });
    ...
    expect(Object.keys(registry.ventures).sort()).toEqual(["kiubo-mexico", "stillface"]);
  });
