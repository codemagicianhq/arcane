...
| SR-05b | **arcane-ui build**: static-export contract (registry, `templateMode`, conformance test, `build-export.tsx`), `src/report/*` per the approved design, trap fixes, `--cat-*` tokens/contrast/print, `ssr.ts` scheme attr, stories + axe — in that repo under its own PRD; release-tagging decision (`ARCUI-016`-shaped) lands here. **Light is in scope, not optional** (operator, 2026-09-03): the report must print, and the light scheme is what prints — so the light lens and the print stylesheet are one deliverable, not two. Carries the `--ink-50`/`--ink-40` light-ramp fix already committed on `fix/light-scheme-ink-ramp-contrast`. The eight category icons ship as v0 drawings from this repo unless arcane-ui replaces them from its own icon set — a template-only change either way, since they are keyed by category, not authored per row (ARC-043). | cross-repo · arcane-ui minor bump + feed publish | SR-05a | conformance test green; `renderToStaticMarkup` snapshot; numeric contrast test **for both schemes on every `--bg-*` ground**; print export checked, not assumed; template ≤ 100 KB raw |
...
**Riskiest assumption — partly retired at SR-05a, and it grew before it shrank.** The stated risk was that the light scheme's contrast retune might exceed what a report epic can carry (Adelaide measured signal/accent at 1.18–2.93:1 on light surfaces). SR-05a measured rather than assumed, and the answer was mixed. *Retired:* the report's own eight `--cat-*` values are solved in both schemes — dark worst 7.92:1, light worst 4.73:1 — so the category palette is no longer an open question in either lens. *Confirmed and worse than described:* the problem is not only signal/accent. `arcane-ui`'s light **ink ramp** fails too — `--ink-40` under AA on all four grounds, `--ink-50` on three — and both carry real product text, so this was a live accessibility defect in the light scheme generally, not a report-specific one. Fixed for those two steps (committed, unpushed); signal/accent remain unmeasured by this program. *Consequence for the fallback:* "ship dark-only first" is no longer available — the operator has made light in-scope because the report must print, and print is light. The residual risk is therefore concentrated in SR-05b's signal/accent retune, which should be measured on every `--bg-*` ground before that epic is sized.
