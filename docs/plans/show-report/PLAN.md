...
## Data contract — `show-report.json` (schema v2)
...
interface ShowReport {
  ...
  provenance: { sources: string[]; compiledAt: string; templateVersion: string };
}
interface Row {
...
**Derived** (never typed by hand): program/frontmatter, stats, outcome, needsYou, section order, `refs`/`href` (PR links in Done notes; `--refresh` snapshot), corrections counts (the program's section of `docs/verification-ledger.md`), parked, close, cast, provenance.
...
> **Schema v2, and the one deliberate break of the freeze (2026-09-05).** The freeze is
> "additive changes only after SR-05b start", and renaming a field is not additive. The operator
> renamed `colophon` → `provenance` anyway, and `schemaVersion` went `1` → `2` rather than leaving
> one version number describing two shapes. Two things made the break cheap enough to take: the
> only consumer of v1 was arcane-ui's compiled template, which was being rebound anyway because it
> never bound correctly in the first place — so **no working consumer of v1 ever existed** — and the
> window closes the moment that template lands. `colophon` is a publishing term for the note on how
> a book was made; `provenance` is the established software term for the same thing, which is what
> `naming-conventions.md`'s own test asks for. No migration path is provided or needed.
...
- New `src/report/`: `ReportMasthead`, `StatRail` (+ `StatTile size="sm"`, `label`), `CategoryLegend` (+ `Badge tone`), `ReportSection` (export the private `PmSection`), `LedgerList`/`LedgerRow` (class-based `.arc-ledger*` CSS, not inline styles — 15 KB vs 24 KB for 40 rows), `ReportProvenance`, `NeedsYou`, `Corrections`, `Cast`; a `ShowReport` composition — exact set subject to the design.
...
| SR-05a | **Design** (Claude Design): `/design-sync` arcane-ui tokens + exportable components into a design-system project; design Show Report against Circe's structure + the JSON model (both lenses: operator / share; light + dark; print); operator approves the design; schema v1 frozen | design · no code | SR-00, SR-01 (real JSON to design against) | **In progress — explored, not yet formalized.** Operator chose a hybrid: explore in Claude Code against the real JSON first, formalize in Claude Design from real components after. Three structural directions were drawn in `arcane-ui`'s actual tokens (values lifted from its `theme.css`, not recalled) against Lessons Hardening's live data — **A Console** (HUD panels, dense rows) chosen over B Briefing (document rhythm) and C Dispatch (verdict-first). **Four decisions came out of the review, all shipped in this repo:** (1) the per-row emoji is dropped — category alone selects the mark, eight inline SVG symbols (**ARC-043**, accepted 2026-09-03 via Q-004); (2) **light is in scope**, because the same report must print, so the light artboard *is* the print master; (3) coverage-at-close needs a committed source before it can be a stat (Q-005); (4) two silent parser defects and one truncation defect, found only because the design used real data, are fixed here. **Also measured, not asserted:** eight `--cat-*` values proposed for both schemes and computed against every ground — dark worst 7.92:1, light worst 4.73:1, all clear of 4.5:1. And the design could not be built on `arcane-ui`'s light ink ramp as it stood: `--ink-50`/`--ink-40` measured 4.36:1 and 3.04:1 on `--bg-1` (down to 2.45:1 on `--bg-3`) while their dark counterparts pass everywhere, so every light-scheme eyebrow and unselected tab in the product was under AA. Retuned to 5.76:1 and 5.05:1, monotonic — committed and **pushed** on `arcane-ui`'s `fix/light-scheme-ink-ramp-contrast` branch (commit `4196518`), held until the operator's explicit go-ahead since arcane-ui is outside this delegation; no PR open there yet. **Remaining:** `/design-sync` + formalize direction A in Claude Design from real components, then freeze schema v1. |
| SR-05b | **arcane-ui build**: static-export contract (registry, `templateMode`, conformance test, `build-export.tsx`), `src/report/*` per the approved design, trap fixes, `--cat-*` tokens/contrast/print, `ssr.ts` scheme attr, stories + axe — in that repo under its own PRD; release-tagging decision (`ARCUI-016`-shaped) lands here. **Operator lens only** (operator, 2026-09-04) — the share lens is deferred to SR-05c. **Light is in scope, not optional**: the report must print, and the light scheme is what prints, so the light lens and the print stylesheet are one deliverable. **Prerequisite, not scope:** `arcane-ui`'s light signal/accent retune lands first as its own item there (see the row below the table); SR-05b builds on a sound light scheme rather than designing around a broken one. The eight category icons and both `--cat-*` palettes **ship as-is from this repo as v0** (operator, 2026-09-04) — arcane-ui may replace them later from its own icon set, a template-only change since they are keyed by category, not authored per row (ARC-043). | cross-repo · arcane-ui minor bump + feed publish | SR-05a, arcane-ui light signal/accent retune | conformance test green; `renderToStaticMarkup` snapshot; numeric contrast test **for both schemes on every `--bg-*` ground**; print export checked, not assumed; template ≤ 100 KB raw |
| ~~SR-05c~~ | **Share lens** (deferred out of SR-05b by the operator, 2026-09-04): the second lens over the same data model — IDs demoted to link text, internal paths stripped, Open Graph head, `spell ward`-gated content, "Built with Arcane" link. Adds an optional `lens` field to the model; the schema freeze permits this because it is **additive**. Deferred because ward-gating report content is ARC-031-class privacy work, not a template variant. | cross-repo · arcane-ui + arcane-cli | SR-05b | **Moved OUT of this program at close (2026-09-05), not delivered.** Tracked in `TODO.md` instead. It was deferred out of SR-05b by the operator on 2026-09-04 and never started. Keeping it inside would block SR-08 indefinitely, since SR-08 depends on *all* — and the share lens is a genuine cross-repo feature (an arcane-ui template variant, an additive `lens` field, a CLI flag, and `spell ward` gating over report content, which is ARC-031-class privacy work), not a finishing touch. Nothing is lost: the schema freeze permits an additive `lens` field whenever it is picked up. |
| SR-06 | Vendor the compiled template into `src/assets/report/` (replacing v0); `check:report-template`; re-baseline the golden test | direct · minor | SR-02, SR-05b | **Done:** `v0.38.0`. arcane-ui **2.1.3**'s compiled template is vendored into `src/assets/report/`, replacing the v0 interim one; both programs regenerate and `check:report` is clean. **It took four arcane-ui builds to get here, and the gate is the reason.** 2.1.0 bound to the raw `show-report.json` instead of `buildShowReportView()`'s output: fourteen tags resolved to empty strings, the `<h1>` rendered empty, section headers all read `W0`, and one program's calibration sentence was hardcoded as static text -- and it passed every gate that existed, because mustache renders an unknown tag as an empty string, silently, and a consistently-wrong template still regenerates byte-identically. 2.1.1 fixed the binding but shipped no `<title>` (WCAG 2.1 SC 2.4.2, Level A, on every report), no viewport meta, and an unlabelled needs-you list. 2.1.2 fixed those but left the footer heading reading **Colophon** -- the exact word ARC-043's rename existed to remove. 2.1.3 is clean. `check:report-template` (new, blocking in CI) is what caught rounds two and three: it renders the template and fails on dead tags, unrendered tags, an empty `<h1>`, a missing title/lang/viewport, a lost provenance label, non-determinism, and a row that renders blank instead of visibly `unwritten`. Dead-tag detection runs against a synthetic maximal model, not the committed reports -- checking real data first reported the correctly-guarded optional `note` as dead. **Verified, not assumed:** 1245 tests green; all three colour schemes measured from the generated file -- dark worst `--cat-*` 7.92:1, light and print worst 5.61:1, every ink step clear of 4.5:1; zero `<script>`; `<title>` present; print block present. **One advisory left:** the cast still renders "1 commits" -- 2.1.3's commit message claims a singularisation that a diff of the built templates shows did not land. Non-blocking by design. **Re-vendored at `v0.38.2`** with arcane-ui **2.1.4**: the last advisory is closed. The template now reads "1 commit" and "1 item" at a count of one, using `cast[].isSingular` and `parkedIsSingular` -- flags arcane-cli had to supply, because mustache has no conditional on a value and the template could not branch on the number alone. Verified at counts 0/1/2/9/21, and on the real Lessons Hardening data, which now reads "21 commits, 1 commit". `check:report-template` is fully silent -- no failures, no warnings. |
| SR-07 | Automation (**operator-confirmed**): arcane-ui pipeline stage runs `build-export`, and opens/updates a PR on `codemagicianhq/arcane` whenever a compiled export changes (GitHub token in an ADO variable group) — the "consistent + automatic" guarantee | platform · no | SR-06 | **Authored, not enabled — the one deliberate deviation at close.** The pipeline stage is written and committed in `arcane-ui` (`azure-pipelines.yml` stage `VendorShowReportTemplate`, branch `feat/sr-07-export-template-pr`), with `docs/sr-07-enable.md` carrying the three operator steps. **It is inert as committed:** its `condition` tests `GITHUB_PR_TOKEN` for emptiness, so with no variable group the stage is skipped and that pipeline behaves exactly as before. It cannot go further without an operator: step 1 mints a fine-grained GitHub token, step 2 stores it as an Azure DevOps secret — both platform mutations excluded from every delegation — and step 3 is a judgement about whether to trust the automation. It is also **untested by construction**, since it cannot run without the credential it is gated on; it is written to be read first. Three details are load-bearing, each from a real failure here: the change comparison ignores the template's first line (which carries the arcane-ui version and so changes every publish, which would otherwise open a no-op PR per release); the PR carries the version bump and regenerated reports, because `check:version-bump` and `check:report` both gate here and a template-only PR cannot go green; and `check:report-template` runs in the pipeline *before* the PR opens, so a broken template fails there rather than being proposed. The stage never merges. **RESOLVED 2026-09-06 — enabled and verified end to end, after the program's close.** The operator created the fine-grained token and the `vg-arcane-ui-vendoring` group; the stage now runs. Proven against the real thing rather than reasoned about: arcane-ui run 627 built the template, detected the change, cloned, bumped, regenerated, passed its own `check:report-template`, pushed, and opened [PR #216](https://github.com/codemagicianhq/arcane/pull/216) here — which went green and merged, putting `arcane-ui v2.1.7`'s template into `src/assets/report/` as `v0.38.3`. **Enabling it exposed two defects in the stage, both fixed:** the gate tested the *secret* (`ne(variables['GITHUB_PR_TOKEN'], '')`), which can never open because a secret is invisible to a YAML expression — it now tests a non-secret `VENDORING_ENABLED` flag (arcane-ui PR !839); and the clone used `--depth 50`, so `fix:report` regenerated **without** `versionSpan` and `cast` and the first bot PR deleted them from both reports — now a full clone with an `is-shallow-repository` guard (PR !841). That second one is the same silent-degradation class as Q-003 and the 2.1.0 template, and it was caught only because this repo's CI checks out `fetch-depth: 0` and therefore disagreed. The stage opens a PR only when the compiled template body actually differs, and never merges. |
| SR-08 | Program DoD audit + close (walk the DoD with evidence, `spell-check-drift`, `spell-close-session`) | process · no | all | **Done:** [PR #214](https://github.com/codemagicianhq/arcane/pull/214), merged 2026-09-05 (`994bc67`). Verdict **GO**; the full close-out audit with per-criterion evidence is in the Close-out audit section below the epic table. Two deviations disclosed rather than dropped: SR-07 authored but inert, SR-05c moved out to `TODO.md`. One criterion deliberately unverified and stated as such: "prints to PDF cleanly". **Report:** Arcane can now write its own closing report: when a program finishes, `spell report` turns its plan and git history into a single self-contained page — what shipped, what was decided, what was deliberately not built, and who did it — rendered in the product's real design system and readable offline, in a browser or on paper. · category: process |

**Riskiest assumption — retired as a report risk, promoted to an `arcane-ui` prerequisite.** The stated risk was that the light scheme's contrast retune might exceed what a report epic can carry (Adelaide measured signal/accent at 1.18–2.93:1 on light surfaces). SR-05a measured rather than assumed. *Retired:* the report's own eight `--cat-*` values are solved in both schemes — dark worst 7.92:1, light worst 4.73:1. *Confirmed and worse than described:* the problem was never only signal/accent. `arcane-ui`'s light **ink ramp** failed too — `--ink-40` under AA on all four grounds, `--ink-50` on three — and both carry real product text, so this was a live accessibility defect in the light scheme generally. Those two steps are fixed and pushed on `fix/light-scheme-ink-ramp-contrast`. *Resolved 2026-09-04:* the operator moved the remaining signal/accent retune **out of SR-05b and into its own `arcane-ui` item, landed before SR-05b starts** — it is product-wide breakage (a green badge is unreadable in light mode), not report scope, and it should not ride inside a feature epic. SR-05b therefore carries no open contrast risk of its own; it inherits a light scheme that has already been measured on every `--bg-*` ground.
...
## Close-out audit (SR-08, 2026-09-05)

Every criterion below was executed at close, not recalled. Verdict: **GO**, with one disclosed
deviation (SR-07) and one item moved out of scope (SR-05c).

> **Update 2026-09-06 — SR-07's deviation is resolved.** The automation was enabled and verified end
> to end the day after this close; see its row in the epic table. Enabling it exposed two defects in
> the stage, both fixed. **SR-05c remains out of scope**, tracked in `TODO.md`.

| DoD criterion | evidence |
|---|---|
| `check:report` green, zero findings on both programs | `npm run check:report` — "every `docs/plans/*/show-report.{json,html}` matches a regeneration from its sources" |
| `spell report` offline in a consumer-shaped repo | `test/report-command.test.ts`, 5 tests green — builds a scratch repo from a copied real `PLAN.md`, no network, no baseline history |
| Golden test regenerates byte-identically | the ARC-012-style parity test in `test/report-cli.test.ts`, plus `check:report` above |
| Artifact: ≤ 100 KB, zero external scripts, fonts only from Google Fonts | 40,973 bytes; **0** `<script>`; the only external hosts are `fonts.googleapis.com`, `fonts.gstatic.com` and `github.com` — the last appearing only as PR link `href`s, never as a loaded resource |
| Dark and light both readable | measured **from the generated file**, not the template: dark worst `--cat-*` **7.92:1**, light and print worst **5.61:1**; `ink-90`/`60`/`50` all clear 4.5:1 in all three blocks |
| Contrast asserted by a numeric test in arcane-ui | `test/theme/contrast.test.ts` — 153 tests, including a dedicated `--cat-*` block covering all eight categories on every `--bg-*` ground in **both** schemes (SREB-001) |
| Full suite + every check | `typecheck`, `lint`, `test` (1246 passing, 2 skipped), `check:self-host-parity`, `check:spell-catalog`, `check:citations`, `check:report`, `check:report-template`, `check:adr-references` — all green |
| Version-bump gate per epic | enforced by `check:version-bump --staged` in the pre-commit hook throughout; `0.34.x → 0.38.2` across the program |

**Not verified, stated rather than implied:** "prints to PDF cleanly". The `@media print` block
exists and its tokens are the light set, every one measured above 4.5:1 — but no PDF was actually
produced and inspected. It is a one-minute check in a browser and it is the operator's to make.

**What this program is really a record of.** Four separate `arcane-ui` builds shipped a compiled
template that every existing gate called green. 2.1.0 bound to the raw JSON instead of
`buildShowReportView()`'s output — fourteen tags resolved to empty strings, the `<h1>` rendered
empty, every section header read `W0`, and one program's calibration sentence was baked in as static
text. `check:report` and the golden test both stayed green throughout, because a template whose tags
name nothing still regenerates *byte-identically*: it is consistently wrong. Mustache renders an
unknown tag as an empty string, silently. `check:report-template` (SR-06) exists because of that and
caught the two rounds after it. **A byte-comparison gate cannot catch a wrong template** is the
single most transferable thing this program learned.

Three generator defects were found the same way — by rendering the live corpus rather than a fixture:
a parked item whose em dash fell on a continuation line was silently dropped (Lessons Hardening
rendered 8 of 9 with nothing to indicate a gap), wrapped reasons were cut mid-sentence, and ledger
claims were truncated mid-word. No test had those shapes; real data did.
