...
| consumer **A** | `1.2.0` | `.github/prompts` + `.agents/skills` + `.claude/commands` + `.arcane/spells` | **two**: `/Spell-Status` and `/spell-status` |
| consumer **B** | `0.38.3` | `.github/prompts` + `.claude/commands`, **no** `.agents/skills` (that location predates 0.39.0) | **two**: `/Spell-Status` from the repo, `/spell-status` from `~/.agents/skills` |
| consumer **C** | `0.35.1` | `.github/prompts` + `.claude/commands` | one Title-case entry from the repo |
| **A** + **C** in one workspace | — | — | **three**: two Title-case (one per folder), one lowercase |
...
2. **Skills deduplicate by name across locations; prompt files do not deduplicate across workspace
   folders.** Inferred, but from three independent observations that agree: consumer **B** (no repo skill)
   showed the lowercase entry resolving to `~/.agents/skills`; consumer **A** (repo skill *and* user tier
...
was retired). Verified on disk: consumers **A** and **C** each have 12 agent files and an
