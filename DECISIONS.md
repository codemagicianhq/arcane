...
| [ARC-003](#arc-003--agent-persona-schema-v2-and-trinity-replaces-jack)                    | Agent Persona Schema v2 and Trinity Replaces Jack                       | 2026-05-18 | Accepted |
...
| ADR-052 | Product Excellence Standards and spell-elevate PRD Quality Enhancement                      | Quality gates                                  |
...
The `@codemagician/arcane` CLI distributes spell prompts and governance templates to consumer repos via `spell init` / `spell update`. However, the `.github/agents/*.agent.md` files — which VS Code Copilot reads to surface named agent modes (Primus, Gandalf, Thor, etc.) in the chat UI — were absent from the asset distribution. Consumer repos that ran `spell init` got spells and governance but no agents appeared in VS Code.

A working reference implementation of all 12 agent files existed in a consumer repo. A corrected set (with the `[object Object]` serialization bug fixed in `flash.agent.md`) was also verified in another consumer repo.

**Problem with the bug:** An earlier version of the agent generation tooling serialized a JavaScript object reference instead of a string in the `flash.agent.md` description field, producing the literal text `[object Object]`. This was caught during the consumer-repo verification pass and corrected before committing to arcane assets.

**Decision:**

1. Add 12 `*.agent.md` files to `src/assets/.github/agents/` — one per Arcane persona: Primus, Gandalf, Thor, Snape, Scotty, Wasp, Flash, Vision, Wanda, Jack, Gideon, Heimdall.
...
## ARC-003 — Agent Persona Schema v2 and Trinity Replaces Jack
...
Additionally, the `operations-comms` agent was originally named Jack (Nightmare Before Christmas), chosen for a specific co-owner comms use case at a venture. That use case was deferred, and the role was generalized to operational communications across all projects — making a more tactically precise persona more appropriate.
...
4. **Trinity replaces Jack** as the `operations-comms` persona. Trinity (The Matrix) — tactical, economical, zero-noise — better matches the general ops-comms role. Jack (Nightmare Before Christmas) moves to the Standby pool in `naming-conventions.md` for potential future re-activation.
5. **CODEMAGICIAN_NAME_MAP** in `naming.ts` updated: `"operations-comms": "Jack"` → `"operations-comms": "Trinity"`.
...
- The `operations-comms` agent will be named Trinity in all generated outputs (VS Code agent mode, IDENTITY.md, etc.).
...
- `src/assets/.arcane/agents/agent-policies-template.md` — 13 Jack references → Trinity
- `src/assets/.arcane/governance/agent-policies.md` — 13 Jack references → Trinity
- `src/assets/.arcane/governance/agent-approved-paths.md` — workspace-jack paths → workspace-trinity
- `src/assets/.arcane/governance/git-conventions.md` — Jack email row → Trinity (date 2026-05-18)
- `src/assets/.arcane/governance/hardening-checklist.md` — Jack in exec-deny list → Trinity
- `src/assets/.github/prompts/spell-present-arcane.prompt.md` — Jack in agent lineup → Trinity
- Mermaid diagram node color updated from Jack purple (`#5a2d5c`) to Trinity Matrix green (`#1a1a1a/#00ff41`)
...
- Trinity (ARC-003) has a net-new image-prompt file; corresponding PNGs must be generated by the downstream consumer.
...
*spell prompt* itself. `product-excellence-standards.md` and `spell-elevate` grade PRDs/products, not
...
- **Reuse spell-elevate** — rejected; it is PRD-specific by design (Vision/Wanda lenses, PRD scorecard)
...
binary** is `spell`, and the **npm package** is `@codemagician/arcane`. The open question was whether the
brand≠binary split is correct, and whether the binary should be renamed to `arcane` before going public.

Verified facts (2026-06-23): `@codemagician/arcane` is **available on public npm**; bare `arcane` is
taken (v2.0.6); `arcane-cli`, `arcane-framework`, and `@arcane/cli` are available; `spell-cli` is taken.
...
1. **Three name surfaces, each named by its own convention — the brand≠binary split is intentional and kept:**
   ...
   - **npm package:** `@codemagician/arcane` (named for product + publisher) — **kept**, not renamed
...
3. **GitHub org: `codemagicianhq`**, repo: `arcane` → `github.com/codemagicianhq/arcane` (created
   ...
   suffix-free public handle — the legal `llc` form is left to the private Azure DevOps org. The org is
   an umbrella for multiple public products (Arcane and, later, `dark-matter-complex`), aligned with the
   `@codemagician/arcane` npm scope.
...
- `spell` is load-bearing, not incidental: the methodology is the **Spell Loop** and the entire prompt
  ...
  (reaffirms ops ADR-061 §1).
- The alias gives the discoverability upside of `arcane` at ~zero cost and zero migration.
- Keeping `@codemagician/arcane` (vs. an unscoped rename) preserves install docs and version-history
  continuity; the scoped name is free on public npm, so a rename buys nothing.
...
- **Rename the npm package to unscoped `arcane-cli` / `arcane-framework`** — costs a rename, resets
  download stats, and breaks existing install docs, with no benefit since `@codemagician/arcane` is
  already available on public npm. Rejected.
