...
- **Show Report** is **complete** ([docs/plans/show-report/PLAN.md](docs/plans/show-report/PLAN.md),
  `status: complete` 2026-09-05, [ARC-042](DECISIONS.md#arc-042--show-report-compiled-template-distribution-model-and-program-decisions)
  and [ARC-043](DECISIONS.md#arc-043--show-report-rows-carry-no-emoji-category-selects-the-mark) accepted). `spell report`
  generates a program's completion report from its own plan and git history, rendered through the
  arcane-ui compiled template vendored at `src/assets/report/`. Two epics did not ship as planned and
  both are recorded rather than quietly dropped: **SR-07** (pipeline automation) is authored but inert,
  waiting on an operator-created credential, and **SR-05c** (the share lens) was moved out to `TODO.md`.
