...
argument-hint: The idea itself — a sentence to a couple of paragraphs (e.g., "add a dark-mode toggle to the settings page"), optionally "for <venture>" (hub only, e.g. "save this idea for kiubo — …")
agent: agent
---
...
If the input names a venture ("save this idea for kiubo", `--venture <slug>`), resolve it through `ventures/registry.json`'s aliases first (exact slug → alias → closest match offered — never guessed). Unknown slug:
...
```
(mentions kiubo-mexico — re-save with --venture kiubo-mexico, or route it later with spell-manifest)
```
