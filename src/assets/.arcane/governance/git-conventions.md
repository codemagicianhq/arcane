...
- `thor/feat/api-endpoint` (agent — Thor)
- `gandalf/docs/architecture-update` (agent — Gandalf)
- `snape/fix/test-regression` (agent — Snape)
...
1. **Create a prefixed topic branch:**
   ```bash
   git checkout -b thor/feat/api-endpoint origin/main
   ```
...
3. **Sync with main before opening a PR** — always rebase on the latest origin/main before pushing
   or creating a PR, regardless of whether conflicts are expected:
   ```bash
   git fetch origin
...
5. **Merge or queue for review** based on power level:
   - **Champion+ agents:** self-merge via `git merge --ff-only`, then push main
   - **Below Champion:** push the branch, report branch name, and queue for human merge
...
| **Autonomous agent** at Champion+ power level                     | May self-commit and self-merge within approved scope (per your power-level policy). Must use proper author/trailers. | Agent is operating independently with delegated authority.    |
| **Autonomous agent** below Champion                               | Must commit to topic branch and queue merge for human review.                                                     | Insufficient autonomy level for unsupervised merges to main.   |
...
| **Pre-PR sync**    | Rebase on latest `origin/main` before pushing. No stale divergence.                                                                            |
...
| Gandalf         | `gandalf@{OPERATOR_DOMAIN}` | Autonomous agent                | CTO / Architecture Lead    |
| Thor            | `thor@{OPERATOR_DOMAIN}`    | Autonomous agent                | Full-Stack Developer       |
...
| -------------- | ----------- | --------------------------------------------- |
| `Agent`        | Yes         | `primus`, `copilot`, `claude`                 |
