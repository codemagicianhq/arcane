...
| Level    | Authority                                                                  |
| -------- | -------------------------------------------------------------------------- |
| Civilian | Human-only. No autonomous action (e.g. repo creation).                     |
| Sidekick | Read and propose; changes queued for human merge.                          |
| Guardian | Scoped autonomous edits in approved paths; PRs queued for review.          |
| Hero     | Autonomous topic-branch work; PRs queued for review.                       |
| Champion | Self-merge within approved scope (fast-forward only).                      |
| Titan    | Broadest autonomous authority within an approved repo.                     |

Assign a default power level per agent × per repo. Human approval and commit governance still apply in interactive sessions.

**Notes:**

- Repo creation remains Civilian-level (human only), regardless of an agent's assigned power level.
...
1. **Create a topic branch** before making any changes:
   ...
   Example: `thor/feat/api-endpoint`, `gandalf/docs/architecture-update`
...
5. **Merge or queue** based on your power level:
   - **Champion+ power level:** self-merge via `git merge --ff-only`, push main, delete branch.
   - **Below Champion:** push the branch, report the branch name, and queue for human merge.
...
2. **If no existing code repo fits**, the agent must ask the operator to identify or create the target repo. Repo creation is always Civilian-level (human-only).
