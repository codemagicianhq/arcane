---
name: Spell — Enchant
description: "Enchant a PRD from functional to exceptional — proactively enhance features, UX, accessibility, security, and performance using a research lens and a marketing/brand lens"
claude_description: Use PROACTIVELY when a PRD should be enhanced — UX, accessibility, security, performance — before implementation.
argument-hint: "Path to PRD, pasted PRD content, or tracker work item ID (e.g., 'Enchant PRD.md for {BUSINESS_NAME} <feature> dashboard')"
agent: agent
---
...
Enchant the provided PRD to exceptional quality.
...
```markdown
## Quality Scorecard — Before Enchantment
...
```markdown
## Research Summary (Research Lens — default: Alexander)
...
- Does the PRD define a "signature feature" that makes the app memorable?
...
```markdown
## Marketing Review (Marketing/Brand Lens — default: Circe)
...
Enhancements that lift a dimension one tier above the profile's target. Not blocking the profile's quality bar but enchanting toward the ceiling.
...
- **Enchantd PRD scope:** [N+X requirements, estimated M+Y stories]
- **Scope increase ratio:** [percentage or multiplier]

**If enchanted scope exceeds 2× the original:**
...
| Metric | Original | Enchantd | Change |
|--------|----------|----------|--------|
...
Generate the enchanted PRD with these characteristics:

1. **All original requirements preserved** — never remove or weaken what was already specified
2. **New requirements tagged `[ENCHANTED]`** — clearly distinguishable from original content
...
- **[ENCHANTED]** [New must-have from enchantment]

### Should Have
- [Original should-haves]
- **[ENCHANTED]** [New should-haves from enchantment]

### Won't Have (this iteration)
- [Original exclusions]
- **[ENCHANTED]** [New explicit exclusions from enchantment]

## User Experience Requirements
**[ENCHANTED section if not in original]**
...
- [ ] **[ENCHANTED]** [New criteria from enchantment]

## Dependencies
[Original + any new dependencies from enchanted requirements]

## Open Questions
[Original + any new questions raised by enchantment]
...
Present the enchanted PRD to the user with:
...
4. **Explicit ask:** "Do you want to proceed with the enchanted PRD, adjust any enhancements, or revert to the original?"

Wait for user approval before the enchanted PRD replaces the original. The user may:
- **Accept all** — proceed with the full enchanted PRD
...
- **Be honest about unknowns.** If competitive analysis requires actual market research that can't be done in this context, flag it as a research item for the research lens (default: Alexander) to investigate separately.
- **Don't gold-plate for the sake of it.** If the PRD already meets the profile's targets across all dimensions, say so and move on. Not every PRD needs enchantment.
...
- **Security enhancement is non-negotiable.** Even if the user skips other enhancements, security gaps identified during enchantment must be flagged prominently. The user can still decide, but they must be aware. Security Bronze is always Must-Add regardless of profile.
